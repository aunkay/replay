...
Short borrowing accrues over elapsed calendar time using the prior observed price and a 365-day year. It reduces cash and account P&L and appears separately from commissions in account totals and session exports. Borrow availability is not modeled. Closed-trade P&L and trade CSV exports include financing attributed to each trade episode.
...
Each parent candle uses the finer sequence only when its opening price, high, low, closing price and total volume reconcile. Missing or incompatible windows retain conservative parent-candle execution; the panel reports both reconciled candles and fallbacks. The finer sequence can resolve a target occurring before a later stop, while ambiguous finer candles still use stop-first handling. Live subscribes to the selected finer interval through the same spaced provider polling. A missing or late finer window uses the conservative fallback and is never retroactively re-executed. Ledger/chart timestamps remain aligned with the parent candle; actual intrabar execution timestamps are retained separately. Trailing rules update at the parent close.
...
Backtests run in a worker, separate from the API's trading engine requests. Inputs are limited to 100,000 candles and parameter searches to 500 combinations. Add parameter paths with minimum, maximum and step. The default chronological split is 70% training / 30% test. Candidates are ranked on training results only; the chosen strategy starts a fresh account on the test segment, retaining earlier candles for indicator warmup. The trailing/staged-exit section adds break-even activation, trailing stops and up to three percentage profit targets to strategy runs. The selected finer execution dataset is used by the strategy engine. Review progress, cancel work, inspect candidate results, compare saved runs and export trades/results. Run snapshots retain strategy inputs, dataset hash and engine version. An interrupted worker is reported explicitly rather than silently rerun.
